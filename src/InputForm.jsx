...
const InputForm = ({ countries, setCountries, heads, head, setHead }) => {
  ...
  const addCountry = () => {
    if (!preventCountry(information.country)) {
      alert("국가를 입력해주세요!");
      return;
    }
    ...
    if (preventMedal(information)) {
      alert("메달을 입력해주세요!");
      return;
    }

    if (!notParticipate(information.country)){
      alert("참가국이 아닙니다!");
      return;
    }
...
//국가 미입력 예방 함수
const preventCountry = (country) => {
  return country || false;
};

//메달 미입력 예방 함수
const preventMedal = (information) => {
  return (
...
//미참가국 예방 함수
const notParticipate = (country) => {
  return COUNTRY_LIST.includes(country);
}
